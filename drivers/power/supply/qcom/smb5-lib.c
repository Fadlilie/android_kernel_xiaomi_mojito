...
static const struct apsd_result *smblib_get_apsd_result(struct smb_charger *chg)
{
	...
	if (apsd_stat & QC_CHARGER_BIT) {
		...
		if (result == &smblib_apsd_results[HVDCP3] && chg->qc3p5_authenticated)
			result = &smblib_apsd_results[HVDCP3P5];
		else if ((result != &smblib_apsd_results[HVDCP3P5]) && (result != &smblib_apsd_results[HVDCP3])) {
...
int smblib_get_fastcharge_mode(struct smb_charger *chg)
{
	...
	rc = power_supply_get_property(chg->bms_psy, POWER_SUPPLY_PROP_FASTCHARGE_MODE, &pval);
	if (rc < 0) {
...
int smblib_set_fastcharge_mode(struct smb_charger *chg, bool enable)
{
	...
	rc = power_supply_get_property(chg->bms_psy, POWER_SUPPLY_PROP_AUTHENTIC, &pval);
	if (rc < 0) {
	...
	if (enable && (pval.intval >= 480 || pval.intval <= 150)) {
			smblib_dbg(chg, PR_MISC, "temp:%d is abort, do not setfastcharge mode\n",
			           pval.intval);
			enable = false;
	}
	pval.intval = enable;
				...
				union power_supply_propval *val)
{
	...
	rc = smblib_get_prop_from_bms(chg, POWER_SUPPLY_PROP_CAPACITY, &batt_capa);
	if (rc < 0)
			       ...
			       union power_supply_propval *val)
{
	...
	if (chg->use_bq_pump) {
		...
		if (usb_present && chg->use_bq_pump &&
		   (get_client_vote_locked(chg->usb_icl_votable, MAIN_ICL_MIN_VOTER) == MAIN_ICL_MIN)) {
			val->intval = true;
			return rc;
		}
	}
...
static void smblib_raise_qc3_vbus_work(struct work_struct *work)
{
	...
	if (usb_present) {
		...
		if (chg->is_qc_class_a) {
			vote(chg->usb_icl_votable, SW_ICL_MAX_VOTER, true, HVDCP_CLASS_A_MAX_UA);
		} else {
			vote(chg->usb_icl_votable, SW_ICL_MAX_VOTER, true, HVDCP_CLASS_B_CURRENT_UA);
			if (chg->support_ffc) {
...
static void smblib_six_pin_batt_step_chg_work(struct work_struct *work)
{
	...
	if (input_present == INPUT_NOT_PRESENT) {
		...
		if (is_client_vote_enabled(chg->fv_votable, SIX_PIN_VFLOAT_VOTER))
			vote(chg->fv_votable, SIX_PIN_VFLOAT_VOTER, false, 0);

		if (is_client_vote_enabled(chg->fcc_votable, SIX_PIN_VFLOAT_VOTER))
			vote(chg->fcc_votable, SIX_PIN_VFLOAT_VOTER, false, 0);
